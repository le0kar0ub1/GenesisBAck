...
{
    internal.sad      = mmu_read32(DMA_IOMEM_GETADDR(2, DMA_IOMEM_SAD_SHIFT));
    internal.dad      = mmu_read32(DMA_IOMEM_GETADDR(2, DMA_IOMEM_DAD_SHIFT));
    internal.count    = mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CNT_SHIFT));
    internal.ctrl.raw = mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CTL_SHIFT));
    internal.sad   &= ((1 << 27) - 1);
    internal.dad   &= ((1 << 27) - 1);
...
static void dma_flush_partial(void)
{
    ...
    if (internal.ctrl.dst_ctrl == 0b11) {
        internal.dad = mmu_read32(DMA_IOMEM_GETADDR(2, DMA_IOMEM_DAD_SHIFT)) & ((1 << 27) - 1);
    }
}
...
static bool dma_timing_check(void)
{
    ...
    switch ((mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CTL_SHIFT)) >> 12) & 0b11)
    {
        case 0b00: // immediate
            return (true);
        case 0b01: // V-Blank
        ...
        case 0b11: // Special : Sound FIFO
            return (false);
            dad = mmu_read32(DMA_IOMEM_GETADDR(2, DMA_IOMEM_DAD_SHIFT));
...
void dma2_transfer(void)
{
    ...
    if (!reload && mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CTL_SHIFT)) & (1 << 9)) {
        dma_flush_partial();
    } else {
        dma_flush_internal();
    }
    ...
    if (!(internal.ctrl.repeat)) {
        mmu_raw_write16(
            ...
            mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CTL_SHIFT)) & ((1 << 15) - 1)
        ); // clear the enabled bit
        reload = true;
        if ((bool)(mmu_read16(DMA_IOMEM_GETADDR(2, DMA_IOMEM_CTL_SHIFT)) & (1 << 14))) { // is IRQ enabled ?
